...
def extract_myer_details():
    ...
    current_display_price = check_product('p','data-automation','product-price-now')
...
def extract_david_jones_details():
    ...
    size = check_product('div','class','size-selection')[4:]
    previous_price = ''
    discount = ''
    row = [title,description,current_display_price,previous_price,colour,size,discount]
    print(row)
...
for product in myer_list:
    ...
    while item_found == 'No':
        ...
        WebDriverWait(browser, 1000000).until(EC.presence_of_element_located((By.XPATH,"//div[@data-automation = 'products-container']")))
...
for product in david_jones_list:
    ...
    while item_found == 'No':
        ...
        if container.find_elements(By.XPATH,f'//a[text() = \"{product}\"]') != []:
            item_found = 'Yes'
            located_item = browser.find_element(By.XPATH,f'//a[text() = \"{product}\"]').click()
...
df = pd.DataFrame(data,columns=['Title','Description','Current Price','Previous Price','Colour','Size','Discount'])
